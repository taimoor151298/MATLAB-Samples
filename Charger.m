function [SOC,  Tf_final, Ts_final, P_pump] = Charger(charge_power, starting_temp)

global pa ca ka H A G U T_inf ps cs ks e h D T_in initial_temp xmesh dx ks_eff 

initial_temp = (starting_temp);
...
m_flow = charge_power*heater_eff/(ca*(T_in - T_inf))
G = m_flow/A;
...
t_hr = tstep/3600;
t_min = tstep/60;
plot(t_hr, transpose(Ts(:,1:2:end)-273.15))
plot(t_min, (Ts(:,1:20:end)))
legend(sprintf('At Tank Height'))
legend(string(round(xmesh(1:20:end),2)))
[leg,att] = legend('show');
...
figure(2)
plot_t = 100;
size(Ts(1:plot_t:end,:))

ax1 = plot(xmesh,transpose((Ts(1:plot_t:end,:))),'-')

hold on

ax2 = plot(xmesh,transpose((Tf(1:plot_t:end,:))), '--')
set([ax1(1) ax2(1)],'Color','b')
set([ax1(2) ax2(2)],'Color','g')
set([ax1(3) ax2(3)],'Color','r')
set([ax1(4) ax2(4)],'Color','m')
set([ax1(5) ax2(5)],'Color','c')
...
E_stored_fluid = [0;m_flow*dt*ca*cumsum((Tf_in-Tf(1:end-1,end)))];
size(E_stored_fluid)
...
for i = 1: length(tstep)
    %disp('run')
    E_stored_time1(i) = abs(sum(ps*cs*A*(1-e)* (Ts(i,:) - Ts(1,:))*dx));
end
E_stored_solid1 = (E_stored_time1);
...
E_stored_solidMWh1 = E_stored_solid1/3.6e9;

size(E_stored_time1)
%plot(t_hr, E_stored_MWh, t_hr, E_stored_solidMWh,t_hr, E_stored_solidMWh1)
plot(t_hr, E_stored_MWh,t_hr, E_stored_solidMWh1);
hold on
SOC = E_stored_solid1/E_des;
...
xlabel('Time')
figure()
plot(SOC, transpose(Ts(:,1:25:end)))
end
